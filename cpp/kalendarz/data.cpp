...
if(y1>y2)
{
    ...
    else s=((dod[0][m2+i]) - (dod[0][m2+i-1]));
    ile+=s;
    }
    return ile;
    }
    ...
    else s=((dod[0][m1+i]) - (dod[0][m1+i-1]));
    ile+=s;
    }
    return ile;
    }
...
else return ile_dni(dod,d2,m2,y2,d1,m1,y1);
}
