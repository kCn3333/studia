...
for(int i=0;i<=V;i++)			// i wypelnia je zerami
  {
    ...
    for(int j=0;j<=V;j++) { GrafTab[i][j]=0; RoutingMatrix[i][j]=0; }
  }
}

bool Graf::Wstaw(unsigned v, unsigned u, unsigned waga)
{
if(GrafTab[v][u]>0) return false; // już jest!
GrafTab[v][u]=waga;
RoutingMatrix[v][u]=u;
GrafTab[u][v]=waga;
...
cout <<"\n\t"<<miasto(v2);
if(tmp!=v2) cout <<" -"<<GrafTab[tmp][v2]<<"-> "<<miasto(tmp);
...
linia.clear();
linia2.clear();
tmp.clear();tmp2.clear();tmp3.clear();
position=0;
}
while (!czytaj.eof());
czytaj.close();
