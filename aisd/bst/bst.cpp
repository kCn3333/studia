...
void BinarySearchTree::insert(int d)
  {
    ...
    t->right = NULL;
    parent = NULL;
    if(isEmpty()) root = t;	// czy nie puste drzewo
    else
      {
	tree_node* curr;
...
int BinarySearchTree::search(int d)
  {
    ...
    while(curr != NULL)
      {
	k++;
	if(curr->data == d) return k;
	else
	  {
	    parent = curr;
	    if(d>curr->data) { curr = curr->right; k++; }
	    else { curr = curr->left; k++;}
	  }
      }
...
cout <<endl<<" #szukanie w tablicy..."<<endl;
czas_start=pomiar_czasu();
for(int i=0; i<ITER; i++)
  {
    tmp=rand();
    for (int i=0; i<DANE; i++)
	{ if(TMP[i]==tmp) break; a++; }
  }
czas_stop=pomiar_czasu();
...
cout <<"\n\tczas: "<<czasBST<<" sek. \n\t\t srednio porownan: "<<n/1000<<endl<<endl;
