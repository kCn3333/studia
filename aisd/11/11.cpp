...
if(d <= 4) 
  {
    for(int n=0;n<2*d;n++) C[n]=0; 
    
    for (i=0;i<d;i++)
    {
      for (int j=0;j<d;j++) C[i+j]+=A[i]*B[j];
    }
    return;
  }
...
{
  z=A[i]; A[i]=A[a_dl-i-1]; A[a_dl-i-1]=z;
}
...
{
  z=B[i]; B[i]=B[b_dl-i-1]; B[b_dl-i-1]=z;
}

i = (a_dl > b_dl) ? a_dl : b_dl;
for(d = 1; d < i; d *= 2);
for(i = a_dl; i < d; i++) A[i] = 0;
for(i = b_dl; i < d; i++) B[i] = 0;
...
if(z != 0) cout <<":: przepelnienie bufora... "<<z<<"\n";

cout <<"\n  A*B = ";
for (i=2*d-1;i>0;i--) if (C[i]!=0) break;
for (;i>=0;i--) cout <<C[i];		// wypisanie C
cout <<"\n\n";
