...
B[2]=-0.03369484597;
	...
	for (int j=0;j<=N;j++) b[j]=4;
	for (int j=0;j<=N;j++) c[j]=1;
	for (int i=0;i<=N;i++) d[i]=hh*(Y[i]-2*Y[i+1]+Y[i+2]);
	
	pochodne.resize(N+1);
	pochodne = LU(a,b,c,d,N);
	
	cout << endl << " Szukanie minimum - metoda Brenta:" 
	     << endl << " ---------------------------------"<< endl;
...
if (losowa2<losowa1) { temp=losowa1; losowa1=losowa2; losowa2=temp; }
