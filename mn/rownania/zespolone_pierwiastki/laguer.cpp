...
for (int iter=1;iter<=MAXIT;iter++)
{
  ...
  if(iter % MT !=0) x=x1;
  else x-=frac[iter/MT]*dx;	// fractional step
}
throw("too many iterations in laguer()");	// try different starting guess
}
...
complex<double> f6(complex<double> x)	// funkcja z zadania 6N
{
return sqrt(2.0-2.0*x*x);
}
...
complex<double> C[5];
complex<double> roots3[5];

A[0]=complex<double>(-4.0,0.0);  // dane do zadania 5N.1
...
B[0]=complex<double>(1.0,0.0);  // dane do zadania 5N.2
B[1]=complex<double>(0.0,1.0);  
B[2]=complex<double>(-1.0,0.0);  
B[3]=complex<double>(0.0,-1.0);  
B[4]=complex<double>(1.0,0.0);  

C[0]=complex<double>(1.0/4.0,0.0);  // dane do zadania 6N
C[1]=complex<double>(-3.0/2.0,0.0);  
C[2]=complex<double>(3.0/4.0,0.0);  
C[3]=complex<double>(1.0/2.0,0.0);  
C[4]=complex<double>(1.0/4.0,0.0);  
...
cout.precision(10);
  ...
  cout <<"\n 5N.1 policzone pierwiastki:\n"<<" -------------------------"<<endl;
  for (int I=0; I<N; I++) cout<<" "<<I+1<<" "<< roots[I]<<endl;
  cout<<endl;

  zroots(B,roots2,5);
  cout <<"\n 5N.2 policzone pierwiastki:\n"<<" -------------------------"<<endl;
  for (int I=0; I<N2; I++) cout<<" "<<I+1<<" "<< roots2[I]<<endl;
  cout<<endl;
  
  zroots(C,roots3,5);
  cout <<"\n 6N rozwiazania ukladu:\n"<<" -------------------------"<<endl;
  for (int I=0; I<N2; I++) cout<<" "<<I+1<<" x="<< roots3[I]<<"  y="<<f6(roots3[I])<<endl;
  cout<<endl;
   return 0;
}