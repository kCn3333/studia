...
g = scale = anorm =0.0;		// hausolder reduction to bidiagonal form
for (i=0;i<n;i++)
{
  l=i+2;
  rv1[i]=scale*g;
...
for(i=n-1;i>=0;i--)		// accumulation of righ-hand transformation
{	
  if( i<n-1)
  {
    if(g != 0.0)
...
for(i=MIN(m,n)-1;i>=0;i--)	// accumulation of left-hand transformation
{
  l=i+1;
  g=W[i];
  for(j=l;j<n;j++)
...
void LUdist()
{
  ...
  for(k = 0; k < n - 1; k++)
  {
    if(fabs(A[k][k]) < eps) throw("#LUdist() < eps");
...
void LUsolve(int k)
{
  ...
  if(fabs(A[n-1][n-1]) < eps) throw("#LUsolve() < eps");
  ...
  for(i= n-2;i>=0;i--)
  {
    ...
    for(j=i+1;j<n;j++) 
      s += A[i][j]*COV[j][k];
    if(fabs(A[i][i]) < eps) throw("#LUsolve() < eps");
