...
void read_socket()
{
	...
	buf[len]='\0';
	cout<<buf;
	if(buf[0]!='2') yes=1;		// jesli smtp zwroci blad
	else yes=0;		
	clear(buf);
}
...
if((sock=socket(AF_INET, SOCK_STREAM, 0))==-1)
    {  perror("socket");  exit(1); }	// utworzenie gnizda
...
if (connect(sock, (struct sockaddr *) &server, sizeof server)==-1)
    { perror("connect"); exit(1); }	// laczy sie z serwerem
...
while(1)
{
if(fgets(tab,SIZE,plik)==NULL) break;	// wysyla zawartosc pliku
...
else { cout <<"#blad wysylania wiadomosci (prawdopodobnie podano zly adres)"<<endl; exit(4); }
